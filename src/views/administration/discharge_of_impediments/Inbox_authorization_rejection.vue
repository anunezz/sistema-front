<template>
<header-section title="Autorización de rechazos" :icon-image="iconModule">
</header-section>
...
<q-table
  ...
  <template v-slot:body="props">
    <q-tr  :props="props" :class="[ props.row.id_prioridad == 1 ? 'bg-red-3 text-black' : 'bg-white']">
      ...
      <q-td>
        {{ (  props.row?.impediment?.length > 0 ? props.row?.impediment[0].numero_impedimento : ''   ) }}
      </q-td>
      <q-td>
        {{ props.row.full_name }}
      </q-td>
      ...
      <div>
        ...
        <div v-if="( props.row.impediment?.length > 0 && props.row.impediment[0].low)">
            <b>Impedimento baja:</b> {{ props.row.impediment[0]?.low.cat_status?.estatus_solicitud || '—' }}
          </div>
      </div>
    </q-td>
      <q-td>
...
const seeDetails = (hash_id) => {
  router.push({name:'FormAuthorizationRejection', params: { hash_id: hash_id }})
};
...
const getRegisters = ( pag = null) => {
  ...
  DischargeImpedimentsServices.get_data_inbox_authorization_rejection({
    ...pagination.value,
  }).then(response => {
      $q.loading.hide();
...
const search = () => {
  ...
  //transaccion
    BinnacleTransaction(20, 1, `Se realizo una busqueda en el módulo de Autorización de rechazos`)
}
const getCatsIndex = () => {
  $q.loading.show();
...
const clearFilters = () => {
  ...
  getRegisters();
      BinnacleTransaction(20, 1, `Se limpiaron los filtros en el módulo de Autorización de rechazos`)
