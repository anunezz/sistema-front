<template>
<header-section title="Autorización de solicitudes" :icon-image="iconModule">
</header-section>
...
<q-table
  ...
  <template v-slot:body="props">
    <q-tr  :props="props" :class="[ props.row.id_prioridad == 1 ? 'bg-red-3 text-black' : 'bg-white']">
      ...
      <q-td style="min-width: 120px; width: 120px; word-break: break-word; white-space: normal;">
...
const seeDetails = (hash_id) => {
  router.push({name:'ValidationAuthorizationRequests', params: { hash_id: hash_id }})
};
...
const getRegisters = ( pag = null) => {
  ...
  DischargeImpedimentsServices.get_data_inbox_authorization_requests({
    ...pagination.value,
  }).then(response => {
      $q.loading.hide();
...
const search = () => {
  ...
  //transaccion
    BinnacleTransaction(3, 1, `Se realizo una busqueda en el módulo de Autorización de Solicitudes`)
}
const getCatsIndex = () => {
  $q.loading.show();
...
const clearFilters = () => {
  ...
  getRegisters();
      BinnacleTransaction(3, 1, `Se limpiaron los filtros en el módulo de Autorización de Solicitudes`)
