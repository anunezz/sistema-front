<template>
<header-section title="validación de bajas" :icon-image="iconModule" />
...
const seeDetails = (hash_id) => {
  router.push({name:'FormValidateLow', params: { hash_id: hash_id }})
};
...
const getRegisters = ( pag = null) => {
  ...
  DischargeImpedimentsServices.get_data_inbox_validate_low({
    ...pagination.value,
  }).then(response => {
      $q.loading.hide();
...
const search = () => {
  ...
  //transaccion
    BinnacleTransaction(5, 1, `Se realizo una busqueda en el módulo de validación de bajas`)
}
const getCatsIndex = () => {
  $q.loading.show();
...
const clearFilters = () => {
  ...
  getRegisters();
      BinnacleTransaction(5, 1, `Se limpiaron los filtros en el módulo de validación de bajas`)
