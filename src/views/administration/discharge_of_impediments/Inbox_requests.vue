<template>
<header-section title="Solicitudes" :icon-image="iconModule">
</header-section>
    ...
    <div class="row justify-end">
      <div class="col-12 col-md-12 col-sm-12 col-xs-12">
        <div class="row justify-end q-gutter-xs">
          <q-btn-group>
            <q-btn
			v-if="( store.user.perfil.id_perfil == 1 || store.user.perfil.id_perfil == 2 || store.user.perfil.id_perfil == 4 || store.user.perfil.id_perfil == 5 )"
              outline
              size="12px"
              color="primary"
...
<!-- - - - - - > style="min-width: 1300px;" < - - - - - para q-table -->
<q-table
    :rows="registers"
    :columns="columns"
    separator="cell"
...
const newRegister = () => {
  router.push({name:'NewFormRequests'})
};

const updateRegister = (hash_id) => {
  router.push({name:'UpdateFormRequests', params: { hash_id: hash_id }})
};
...
const search = () => {
  ...
  //transaccion
    BinnacleTransaction(2, 1, `Se realizo una busqueda en el módulo de solicitudes`)
}
...
const clearFilters = () => {
  ...
  getRegisters();
      BinnacleTransaction(2, 1, `Se limpiaron los filtros en el módulo de solicitudes`)
