<template>
<header-section title="Validación de Alta por modificación de datos personales" :icon-image="iconModule">
</header-section>
...
const updateRegister = (hash_id) => {
  router.push({name:'FormValidateHighModifyImpediments', params: { hash_id: hash_id }})
};
...
const getRegisters = ( pag = null) => {
  ...
  DischargeImpedimentsServices.get_data_inbox_validate_high_modify({
    ...pagination.value,
  }).then(response => {
      $q.loading.hide();
...
const search = () => {
  ...
  //transaccion
    BinnacleTransaction(21, 1, `Se realizo una busqueda en el módulo de Validación de Alta por modificación de datos personales`)
}
...
const clearFilters = () => {
  ...
  getRegisters();
      BinnacleTransaction(21, 1, `Se limpiaron los filtros en el módulo de Validación de Alta por modificación de datos personales`)
