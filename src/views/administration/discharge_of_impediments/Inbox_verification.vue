<template>
<header-section title="Verificaciones" :icon-image="iconModule" />
  ...
  <template v-slot:body="props">
    <q-tr :props="props" :class="[ props.row.id_prioridad == 1 ? 'bg-red-3 text-black' : 'bg-white']">
      ...
      <q-td style="min-width: 200px; width: 200px; word-break: break-word; white-space: normal;">
        <div v-if="props.row.causales != null" style="white-space: normal;">
          {{ truncateText(props.row.causales[0]?.causal?.causal_impedimento,50) }}
        </div>
...
const seeDetails = (hash_id) => {
  router.push({name:'FormVerification', params: { hash_id: hash_id }})
};
...
const getRegisters = ( pag = null) => {
  ...
  DischargeImpedimentsServices.get_data_inbox_verification({
    ...pagination.value,
  }).then(response => {
      $q.loading.hide();
...
const search = () => {
  ...
  //transaccion
    BinnacleTransaction(6, 1, `Se realizo una busqueda en el módulo de Verificaciones`)
}
...
const clearFilters = () => {
  ...
  getRegisters();
      BinnacleTransaction(6, 1, `Se limpiaron los filtros en el módulo de Verificaciones`)
