<template>
	<header-section title="Autorización de bajas de impedimentos" url-redirect="InboxAuthorizationLowImpediments" :icon-image="iconModule" />
	<br>
...
<q-dialog v-model="dialog" persistent>
    ...
    <q-form @submit.prevent="validateFields(),action_save_form=4" ref="rejectForm">
        <q-card-section>
            <q-input
            type="textarea"
            outlined
			  ...
			  <div class="col-7 q-pt-xs">
				<q-btn-group class="full-width">
				  <q-btn
					...
					color="red"
					class="full-width"
					@click="$router.push({name:'InboxAuthorizationLowImpediments'})"
					label="Cancelar">
				  </q-btn>
				  <q-btn
				  	@click="dialog=true"
				  ...
				  <q-btn
				  	v-if="id_estatus_solicitud == 50"
					@click="validateFields(), action_save_form = 1"
					outline
					color="primary"
					class="full-width"
					label="Actualizar y por confirmar">
				  </q-btn>
				  <q-btn
				  	v-if="id_estatus_solicitud == 250 && id_estatus_solicitud != 300"
					@click="validateFields(), action_save_form = 3"
					outline
					color="primary"
					class="full-width"
					label="Actualizar y Baja">
				  </q-btn>
				</q-btn-group>
...
const rejectForm = ref(null);
	...
	const actionForm = () => {
		...
		switch (action_save_form.value) {
			case 1:
			select_funtion = onChangeStatusConfirm();
			break;
			case 2:
			...
			break;
			case 4:
				if( store.user.perfil.id_perfil == 5 ){
					select_funtion - RejectResponseImpediment();
				}
				if( store.user.perfil.id_perfil == 4 ){
					select_funtion = RejectAuthorizationImpediment();
				}
			break;
		}
	...
	const RejectAuthorizationImpediment = () => {
		rejectForm.value.validate().then(valid => {
			...
			//DischargeImpedimentsServices.change_status_decline({
				...
				textColor: 'white',
				icon: 'cloud_done',
				message: 'Éxito, la solicitud / impedimento ha sido rechazada correctamente.'
				});
				router.push({name:'InboxAuthorizationLowImpediments'});
				}
			}).catch(e => {
				$q.loading.hide();
	...
	const RejectResponseImpediment = () => {
		rejectForm.value.validate().then(valid => {
			if (valid) {
			$q.loading.show();
			DischargeImpedimentsServices.reject_response_impediment({
				observaciones: observaciones.value,
				hash_id: router.currentRoute.value.params.hash_id,
				moduleId: 8,
			}).then(response => {
  ...
  const onChangeStatus = async () => {
	$q.loading.show();
	DischargeImpedimentsServices.send_to_active_low({
		hash_id : router.currentRoute.value.params.hash_id,
		moduleId: 8,
		...form.value
	}).then(response => {
		...
		if (response.data.success) {
			$q.notify({
				...
				textColor: 'white',
				icon: 'cloud_done',
				message: 'Éxito, Se ha enviado inactivo la solicitud / impedimento correctamente.'
			});
			router.push({name:'InboxAuthorizationLowImpediments'});
		}
	}).catch(e => {
  ...
  const onChangeStatusConfirm = async () => {
	$q.loading.show();
	DischargeImpedimentsServices.send_to_confirm_low({
		hash_id : router.currentRoute.value.params.hash_id,
		moduleId: 8,
		...form.value
	}).then(response => {
		...
		if (response.data.success) {
			$q.notify({
				...
				textColor: 'white',
				icon: 'cloud_done',
				message: 'Éxito, Se ha enviado por confirmar la solicitud / impedimento correctamente.'
			});
			router.push({name:'InboxAuthorizationLowImpediments'});
		}
	}).catch(e => {
  ...
  const validateFields = async () => {
	await myForm.value.validate().then(success => {
	  ...
	  switch (action_save_form.value) {
		case 1:
		  title.value = 'Actualizar y por confirmar';
		  message.value = `¿Estás completamente seguro de actualizar y enviar a confirmar la solicitud / impedimento?`;
		  label_btn.value = 'Actualizar y por confirmar';
		break;
		case 2:
		  title.value = 'Actualizar solicitud / impedimento';
		  message.value = '¿Estás completamente seguro de guardar la solicitud / impedimento?';
		  label_btn.value = 'Actualizar';
		break;
		case 3:
		  title.value = 'Actualizar y baja';
		  message.value = `¿Estás completamente seguro de actualizar y confirmar la baja de la solicitud / impedimento?`;
		  label_btn.value = 'Actualizar y baja';
		break;
		case 4:
