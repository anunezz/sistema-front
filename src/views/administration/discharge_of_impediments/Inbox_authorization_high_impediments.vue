<template>
<header-section title="Autorización de alta de impedimentos" :icon-image="iconModule" />
...
<q-table
  ...
  <template v-slot:body="props">
    <q-tr :props="props" :class="[ props.row.id_prioridad == 1 ? 'bg-red-3 text-black' : 'bg-white']">
      ...
      <q-td>
        {{ props.row.impediment[0]?.numero_impedimento }}
      </q-td>
      <q-td>
        {{ props.row.full_name }}
      </q-td>
      ...
      <div>
        ...
        <div v-if="props.row.impediment && props.row.impediment.length">
          <b>Impedimento:</b> {{ props.row.impediment[0]?.cat_status?.estatus_solicitud || '—' }}
        </div>
      </div>
    </q-td>
      ...
      <q-td>
        <div class="flex justify-center">
          <q-btn-group>
            ...
            <q-btn  v-if="props.row.id_estatus_solicitud != 300"
            color="positive"
              size="sm"
              @click="printImpediment(props.row?.impediment[0].hash_id)">
...
const seeDetails = (hash_id) => {
  router.push({name:'FormAuthorizationHighImpediments', params: { hash_id: hash_id }})
};
...
const getRegisters = ( pag = null) => {
  ...
  DischargeImpedimentsServices.get_data_authorization_high_impediments({
    ...pagination.value,
  }).then(response => {
      $q.loading.hide();
...
const search = () => {
  ...
  //transaccion
    BinnacleTransaction(7, 1, `Se realizo una busqueda en el módulo de Autorización de alta de impedimentos`)
}
