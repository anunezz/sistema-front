<template>
<header-section title="Respuesta de impedimentos" :icon-image="iconModule" />
...
<q-table
  ...
  <template v-slot:body="props">
    <q-tr :props="props" :class="[ props.row.id_prioridad == 1 ? 'bg-red-3 text-black' : 'bg-white']">
      ...
      <q-td>
        {{ (  props.row?.impedimento ? props.row?.impedimento.numero_impedimento : ''   ) }}
      </q-td>
      <q-td>
        {{ props.row.full_name }}
      </q-td>
      ...
      <q-td style="max-width: 200px;min-width: 200px; width: 200px; word-break: break-word; white-space: normal;">
      ...
      <div>
        ...
        <div v-if="( props.row?.impedimento && props.row?.impedimento?.low)">
      ...
      <q-td>
        <div class="flex justify-center">
          <q-btn-group>
            ...
            <q-btn v-if="props.row.id_tipo_solicitud != 3 && props.row.impedimento"
            color="positive"
              size="sm"
...
const seeDetails = (hash_id) => {
  router.push({name:'FormResponseImpediments', params: { hash_id: hash_id }})
};
...
const getRegisters = ( pag = null) => {
  ...
  DischargeImpedimentsServices.get_data_response_impediments({
    ...pagination.value,
  }).then(response => {
      $q.loading.hide();
...
const search = () => {
  ...
  //transaccion
    BinnacleTransaction(9, 1, `Se realizo una busqueda en el módulo de Respuesta de impedimentos`)
}
const getCatsIndex = () => {
  $q.loading.show();
...
const clearFilters = () => {
  ...
  getRegisters();
      BinnacleTransaction(9, 1, `Se limpiaron los filtros en el módulo de Respuesta de impedimentos`)
