<template>
	<header-section title="Validación de alta de modificación" url-redirect="InboxValidateHighModifyImpediments" :icon-image="iconModule">
	</header-section>
	<br>
...
<q-dialog persistent v-model="printImpedimentModal" >
  <q-card style="width: 100%; max-width: 900px">
    ...
    <q-card-section>
       <div class="row q-col-gutter-xs">
         <div class="col-md-12 q-pa-md">
            ...
            Se creó un nuevo impedimento con el número: {{ printImpedimentData?.numero_impedimento }}
            </h5>
          </div>
       </div>
    ...
    <q-card-actions align="right">
      <q-btn flat label="Cerrar" @click="$router.push({name:'InboxValidateHighModifyImpediments'})" style="background-color: #691C32;" class="text-white"/>
	  ...
	  <q-dialog v-model="dialog_search" persistent>
		  ...
		  <q-card-section v-if="aux == 1">
			  <q-table
				  ...
				  <q-tr :props="props" :class="[ props.row.urgencia ? 'bg-red-3 text-black' : 'bg-white']">
					  <q-td>
					  {{ props.row.impediment.numero_impedimento }}
					  </q-td>
					  <q-td>
					  {{ props.row.full_name }}
					  </q-td>
					  ...
					  <q-td>
					  {{ props.row.entidad_federativa_nacimiento }}
					  </q-td>
					  <q-td>
					  <ul>
						  <li v-for="(itm, idx) in props.row.impediment.documents" :key="idx">
		  ...
		  <q-card-actions align="right">
			  ...
			  <q-btn label="Nuevo impedimento" outline color="primary" v-if="aux_dependencia" @click="onDependencia" />
			  </q-btn-group>
		  </q-card-actions>
			  ...
			  <div class="col-7 q-pt-xs">
				  <q-btn-group class="full-width">
					  <q-btn
						  ...
						  color="red"
						  class="full-width"
						  @click="$router.push({name:'InboxValidateHighModifyImpediments'})"
						  label="Cancelar">
					  </q-btn>
					  <q-btn
						  outline
  ...
  import ExpandedImpedimentRow from 'components/ExpandedImpedimentRow.vue'
  ...
  const columns = ref([
	...
	},
	{
	  ...
	  field: row => row.entidad_federativa_nacimiento
	},
	{
	  name: 'full_name',
	  align: 'left',
  ...
  const CreateImpedimentModifySave = async () => {
	  ...
	  moduleId: 21,
		  ...
		  router.push({name:'InboxValidateHighModifyImpediments'});
		  }
	  }).catch(e => {
		  $q.loading.hide();
		  ...
		  if (response.data.success) {
			  ...
			  switch (response.data.aux) {
				  case 1:
					  ...
					  dialog_search.value = true;
					  registers.value = response.data.ImPersonas.data;
					  console.log("registers.value: ",registers.value);
				  break;
				  case 2:
					  action_save_form.value = 3;
					  dialog2.value = true;
					  title.value = 'Nuevo impedimento';
					  message.value = 'No se encontraron impedimentos !! ¿Estás completamente seguro de crear un impedimento?';
	...
	await myForm.value.validate().then(success => {
		...
		switch (action_save_form.value) {
			...
			case 3:
				dialog2.value = true;
				title.value = 'Nuevo impedimento';
				message.value = '¿Estás completamente seguro de crear un nuevo impedimento?';
				label_btn.value = 'Nuevo';
			break;
			case 5:
				dialog2.value = true;
  ...
  const actionForm = () => {
	  ...
	  switch (action_save_form.value) {
		  ...
		  break;
		  case 3:
			  retun_event = CreateImpedimentModify();
		  break;
		  case 4:
			  retun_event = CreateImpedimentModifySave();
		  break;
		  case 5:
  ...
  const CreateImpedimentModify =  () => {
	  $q.loading.show();
	  DischargeImpedimentsServices.create_impediment_modify({
		  hash_id : router.currentRoute.value.params.hash_id,
	  moduleId: 21,
	  }).then(response => {
