<template>
  <header-section title="Autorización de solicitudes" url-redirect="InboxAuthorizationRequests" :icon-image="iconModule">
  </header-section>
  <br>
            ...
            <div class="col-10 q-pt-xs">
              <q-btn-group class="full-width">
                <q-btn
                  ...
                  color="red"
                  class="full-width"
                  @click="$router.push({name:'InboxAuthorizationRequests'})"
                  label="Cancelar">
                </q-btn>
                <q-btn
                  outline
...
const type_request_label_btn = computed(() => {
	let label = options.value.cat_tipo_solicitud.find(item => item.id_tipo_solicitud === form.value.id_tipo_solicitud);
	label = ( label == undefined ? '' : label.tipo_solicitud );
	return `Actualizar y solicitar ${label} de impedimento`;
});
  ...
  rejectForm.value.validate().then(valid => {
    if (valid) {
      ...
      DischargeImpedimentsServices.send_to_pending({
        ...
        router.push({name:'InboxAuthorizationRequests'});
        }
      }).catch(e => {
        $q.loading.hide();
...
const validateFields = async () => {
  await myForm.value.validate().then(success => {
    ...
    switch (action_save_form.value) {
      ...
      break;
      case 2:
        title.value = type_request_label_btn.value;
        message.value =`¿Estás completamente seguro de ${type_request_label_btn.value}?`;
        label_btn.value = type_request_label_btn.value;
      break;
      case 3:
...
function onChangeStatus () {
  $q.loading.show();
  DischargeImpedimentsServices.send_to_validate({
	  ...form.value,
    hash_id : router.currentRoute.value.params.hash_id,
    moduleId: 3
  }).then(response => {
