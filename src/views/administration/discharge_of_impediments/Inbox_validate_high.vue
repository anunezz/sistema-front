<template>
<header-section title="Validación de altas" :icon-image="iconModule" />
...
const getRegisters = ( pag = null) => {
  ...
  DischargeImpedimentsServices.get_data_inbox_validate_high({
    ...pagination.value,
  }).then(response => {
      $q.loading.hide();
...
const search = () => {
  ...
  //transaccion
    BinnacleTransaction(4, 1, `Se realizo una busqueda en el módulo de Validación de altas`)
}
...
const clearFilters = () => {
  ...
  getRegisters();
      BinnacleTransaction(4, 1, `Se limpiaron los filtros en el módulo de Validación de altas`)
