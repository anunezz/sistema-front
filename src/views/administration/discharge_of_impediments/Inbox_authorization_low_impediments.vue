<template>
<header-section title="Autorización de bajas de impedimentos" :icon-image="iconModule" />
...
<Modal v-model="dialog"  @keyup.enter="getRegisters()" title="Filtros Avanzados">
  <div class="q-pa-md">
    <div class="row q-col-gutter-xs">
      ...
      <div class="col-md-12 q-pa-md" v-if="[5].includes(store.user.perfil.id_perfil)">
          <q-select
            @new-value="newValue"
            dense
            use-chips
            ...
            emit-value
            map-options
            v-model='filters.id_estatus'
            :options='options.cat_estatus'
...
<q-table
  ...
  <template v-slot:body="props">
    <q-tr :props="props" :class="[ props.row.id_prioridad == 1 ? 'bg-red-3 text-black' : 'bg-white']">
      ...
      <q-td>
        {{ props.row.impedimento.numero_impedimento }}
      </q-td>
      <q-td>
        {{ props.row.full_name }}
      </q-td>
      ...
      <q-td>
        <div>
          <div>
            <b>Solicitud:</b> {{ props.row.cat_status?.estatus_solicitud || '—' }}
          </div>
          <div v-if="props.row.impedimento">
            <b>Impedimento:</b> {{ props.row.impedimento?.cat_status?.estatus_solicitud || '—' }}
          </div>
          <div v-if="props.row.impedimento.low">
            <b>Impedimento baja:</b> {{ props.row.impedimento?.low.cat_status?.estatus_solicitud || '—' }}
          </div>
        </div>
      </q-td>
      ...
      <q-td>
        <div class="flex justify-center">
          <q-btn-group>
            ...
            color="positive"
              size="sm"
              @click="printImpediment(props.row?.impedimento.hash_id)">
			...
			<q-btn
              color="primary"
              size="sm"
              @click="downloadRequestReport(props.row.hash_id)"
              icon="download"
				>
				<q-tooltip anchor="top middle" self="center middle">
				Descargar Formato
...
import imageRoute from 'src/assets/icons/Home/bandeja_autorizacion_baja_impedimentos.png';
...
const seeDetails = (hash_id) => {
  router.push({name:'FormAuthorizationLowImpediments', params: { hash_id: hash_id }})
};
...
const printImpediment = (id) => {
  ...
  $q.loading.show();
    ...
    moduleId: 8,
      ...
      link.download = `impedimento_.pdf`;
      link.click();
...
const getRegisters = ( pag = null) => {
  ...
  DischargeImpedimentsServices.get_data_authorization_low_impediments({
    ...pagination.value,
  }).then(response => {
      $q.loading.hide();
...
const search = () => {
  ...
  //transaccion
    BinnacleTransaction(8, 1, `Se realizo una busqueda en el módulo de Autorización de bajas de impedimentos`)
}
const getCatsIndex = () => {
  $q.loading.show();
  DischargeImpedimentsServices.get_cats_index({
    ...
    console.log(response.data.Results.ImPersonaSolicitud);
      ...
      if (response.data.success) {
        ...
        options.value.cat_estatus = response.data.Results.cat_estatus.filter(e =>
        [50, 250].includes(Number(e.id_estatus_solicitud))
        );
...
const clearFilters = () => {
  ...
  getRegisters();
      BinnacleTransaction(8, 1, `Se limpiaron los filtros en el módulo de Autorización de bajas de impedimentos`)

}

// function filterFnCatEstatus(val, update) {
//   if (val === '') {
//     update(() => { options.value.cat_estatus = catalogsOptions.value.cat_estatus })
//     return
//   }
...
const downloadRequestReport = (hash_id) => {
$q.loading.show();
	DischargeImpedimentsServices.download_request_report({
		id_solicitud : hash_id,
    is_hash:true,
    moduleId: 8,
	},{ responseType: 'blob' }).then(response => {
		$q.loading.hide();
      const type = String(response.headers?.['content-type'] || '');
      console.log('tyoeee', type);
      const blob = new Blob([response.data], { type: 'application/pdf' });
      const link = document.createElement('a');
      link.href = window.URL.createObjectURL(blob);
      ...
      $q.notify({
        ...
        textColor: 'white',
        icon: 'cloud_done',
        message: 'Éxito, se descargó la solicitud correctamente.'
      });
	}).catch(e => {
    $q.loading.hide();
		console.error("error: ",e);
