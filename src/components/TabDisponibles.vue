<template>
    ...
    <q-card style="width: 100%; max-width: 900px;" class="q-pa-sm">
    <!-- Encabezado con título y botón cerrar -->
    <div class="text-h6 pt-3">Asignar solicitud</div>
    <q-form @submit.prevent="assingRequests" ref="assignRequestForm">
...
title="Filtros Avanzados">
    ...
    <div>
        ...
        <template v-slot:body="props">
            ...
            <q-td>
                <div class="q-mb-sm">
                <q-checkbox v-model="selectedItems" :val="props.row.id_solicitud" />
                </div>
            </q-td>
...
import { BinnacleTransaction } from 'src/utils/binnacle';
    ...
    const assignRequestForm = ref(null)
...
const selectedItems = ref([]);
    ...
    const allIds = registers.value.map(row => row.id_solicitud);
    return allIds.every(id => selectedItems.value.includes(id));
});

    const toggleButtonLabel = computed(() =>
    allVisibleSelected.value ? 'Deseleccionar todos' : 'Seleccionar todos'
    );
...
const selectAllVisible = () => {
    ...
    if (allVisibleSelected.value) {
        // Si todos están seleccionados, quitarlos
        selectedItems.value = selectedItems.value.filter(id => !allIds.includes(id));
    } else {
        // Si no todos están seleccionados, agregarlos
        selectedItems.value = Array.from(new Set([...selectedItems.value, ...allIds]));
    }
};
...
function filterFnCatCausal(val, update) {
    ...
    assignRequestForm.value.validate().then(valid => {
        if(valid){
            if(selectedItems.value.length) {
                DischargeImpedimentsServices.assign_requests({
                selectedItems: selectedItems.value,
                id_usuario: filters.value.id_usuario
                }).then(response => {
                    $q.loading.hide();
                    ...
                    textColor: 'white',
                    icon: 'cloud_done',
                    message: 'Éxito, se han asignado las solicitudes correctamente.'
                    });
                    dialog2.value = false
                    selectedItems.value = []
                    getRegisters()
                ...
                $q.notify({
                    type: 'negative',
                    message: 'No se han seleccionado solicitudes.',
                    position: 'top',
                    timeout: 3000,
                    icon: 'error'
                });
